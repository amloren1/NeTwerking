...
class UserFriends(Resource):
    ...
    def get(self, uuid):
        ...
        return {"items": user.get("friends", []), "total_items": len(user.get("friends", []))}
    ...
    def post(self, uuid):
        ...
        friendship_hash = generate_friendship_hash(str(current_user["_id"]), str(friend["_id"]))
        ...
        mongo.db.users.update_one({"_id": current_user["_id"]}, {"$push": {"friends": friend["_id"]}})

        # Add user to friend's friend list along with hash
        mongo.db.users.update_one({"_id": friend["_id"]}, {"$push": {"friends": current_user["_id"]}})

        # Add the friendship hash to the friends
        mongo.db.friends.insert_one({"friendship_hash": friendship_hash, "user1_id": sorted_ids[0], "user2": sorted_ids[1]})
